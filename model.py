...
class LogGaussMF(layers.Layer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.mu_regularizer = tf.keras.regularizers.get(mu_regularizer)
        self.beta_regularizer = tf.keras.regularizers.get(beta_regularizer)
        self.mu_constraint = tf.keras.constraints.get(mu_constraint)
    ...
    def get_config(self):
        ...
        config.update({
            ...
            'mu_regularizer': self.mu_regularizer,
...
def normalized_product_fn(x):
    x = tf.keras.ops.sum(K.log(x + 1e-8), axis=2)
    return tf.keras.ops.exp(x - K.max(x, axis=1, keepdims=True))
